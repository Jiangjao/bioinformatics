...
char *test_list_push() {
    ...
    mu_assert(list->count == 1, "List_push of null did not return null");
    ...
    mu_assert(list_first->first->next == list->last,
	...
	mu_assert(list->first->next->next == list->next,
	...
	mu_assert(list->last->prev->next == list->last,
			"list_push 3 did not modify element's prev node's next value");
...
char *test_list_pop() {
   ...
   mu_assert(list->last->next == NULL, "List_pop pf two elements did not make last element's next node null");
   mu_assert(list->count == 1, "List_pop of two elements did return correct count");
   ...
   mu_assert(list->last->value == secondElement, "List_pop of three elementsdid not modify last element");
...
char *test_list_unshift() {
    ...
    mu_assert(list->last->next == NULL, "List_unshift with one element changed node's next node");
    ...
    mu_assert(list->first->next->prev == list->first, "List_unshift with two elements did not change second element's next node");
    ...
    mu_assert(list->last->prev->next == list->last, "List_unshift with two elements last node's prev's next node");
    mu_assert(list->last->prev->prev == list->first, "List_unshift with two elements changed last node'd prev'd node");
...
char *test_list_shift() {
   ...
   List_push(list, firstElement);
   data = List_shift(list);
   mu_assert(data == firstElement, "List_shift of one element did not return firt element");
   ...
   mu_assert(list->first == list->last, "List_shift of two elements did not make first elements last");
   mu_assert(list->first->value == secondElement, "List_shift of two elements modified first element's value");
...
char *test_list_remove() {
   ...
   mu_assert(list->first == list->last, "List_remove(first) with two elements did not make first and last node equal");
   ...
   mu_assert(data == secondElement, "List_remove(last) with two elements did not return correct element");
   mu_assert(list->first == list->last, "List_remove(last) with two elements did not make first and last equal");
   mu_assert(list->last->value == firstElement, "List_remove(last) with two elements did not keep correct element");
   mu_assert(list->last->prev == NULL, "List_remove(last) with two element did not remove prev node");
   mu_assert(list->last->next == NULL, "List_remove(last) with two element did not remove next node");
   ...
   mu_assert(data == secondElement, "List_remove(middle) with three elements did not return correct element");
   mu_assert(list->first->value == firstElement, "List_remove(middle) with three elements modified first element");
   mu_assert(list->last->value == thirdElement, "List_remove(middle)  with three elements modified last element");
   mu_assert(list->first->next == list->last, "List_remove(middle) with three elements did not change first node's next node");
   mu_assert(list->last->prev == list->first, "List_remove(middle) with three elements did not change last node's prev node");
   ...
   mu_assert(list->first->value == secondElement, "List_remove(first) with three elements did not change first node's value");
   mu_assert(list->last->value == thirdElement, "List_remove(first) with three elements modified node's value");
   mu_assert(list->first->prev == NULL, "List_remove(first) with three elements did not change first node's prev node");
   mu_assert(list->first->next == list->last, "List_remove(first) with three elements did not change first node's next node");
   mu_assert(list->last->prev == list->first, "List_remove(first) with threee elements modified last node's prev node");
   mu_assert(list->last->next == NULL, "List_remove(first) with three elements modified last node's next node");
   ...
   mu_assert(list->first->value == firstElement, "List_remove(last) with three elements did not change first node's value");
   mu_assert(list->last->value == secondElement, "List_remove(last) with three elements modified last node's value");
   mu_assert(list->first->prev == NULL, "List_remove(first) with three elements did not change first node's prev node");
   mu_assert(list->first->next == list->last, "List_remove(first) with three elements did not change first node's next node");
   mu_assert(list->last->prev == list->first, "List_remove(first) with three elements did not change last node's prev node");
   mu_assert(list->last->next == NULL, "List_remove(first) with three elements modified last node's next node");
...
char *all_test(){
   ...
   my_run_test(test_list_create);
   my_run_test(test_list_count);
   my_run_test(test_list_first);
   my_run_test(test_list_last);
   my_run_test(test_list_push);
   my_run_test(test_list_pop);
   my_run_test(test_list_unshift);
   my_run_test(test_list_shift);
   my_run_test(test_list_remove);
   my_run_test(test_list_destroy);
