...
int list_files(glob_t *pglob) {
    ...
    while (fgets(line, MAX_LINE - 1, file) != NULL) {
        ...
        rc = glob(line, golb_flags, NULL, pglob);
        check(rc == 0 || rc == GLOB_NOMATCH, "Failed to glob.");

        // dumb work around to s stupid design in glob
        if (golb_flags == GLOB_TILDE) golb_flags |= GLOB_APPEND;
    }
...
int scan_file(const char *filename, int use_or, int search_len, char *search_for[]) {
    ...
    // read each line of the file and search thatr line for the contents
    while (fgets(line, MAX_LINE - 1, file) != NULL) {
...
int main(int argc, char *argv[]) {
    ...
    check(list_files(&files_around) == 0, "Failed to list files.");

    for (i = 0; i < files_around.gl_pathc; i++) {
        scan_file(files_around.gl_pathv[i], use_or, argc, argv);
    }

    globfree(&files_around);
    return 0;
