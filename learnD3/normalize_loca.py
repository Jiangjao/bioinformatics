...
class ParetoNormalizer(Normalization):
	...
	def transform(self, data):
		...
		for row in data_input:
			...
			data_mean = statistics.mean(row)
			data_std = 	Decimal(math.sqrt(statistics.stdev(row)))
	...
	def transform(self, data):
		...
		for row in data_input:
			...
			try:
				...
				row_total = Decimal(str(row_total))
				print("row_total >>>", row_total)
				# mean = [x / Decimal(row_total)  for x in row if x]
				mean = [float(x / row_total)
...
def find_numeric_column(file_path):
	# 读入CSV文件
	with open(file_path) as f:
		...
		for i, col in enumerate(headers):
			try:
				float(reader.fieldnames[i]) 
				numeric_cols_index.append(i)
			except ValueError:
				pass
		
	print(numeric_cols_index)
...
def parse_data_2_numeric_nonnumeric(data):
	"""
	data: matrix
	"""
	column_header = data[0]
	column_has_number = [True] * len(column_header)

	for i in range(len(column_header)):
		for row in data[1:]:
			if not is_number(row[i]):
				column_has_number[i] = False
	# print(column_has_number)    # 将表格字符串转换为列表形式

	# 
	# mask_matrix = [[None] * len(column_has_number) for _ in range(len(data))]
	column_has_number[0] = True
	column_dict = {}
	for i in range(len(column_header)):
		column_dict.setdefault(column_header[i], i) 
	...
	for i in range(len(data)):
		new_data = data[i]
		new_row = []
		new_row_with_nonnumeric_without_first_item = []
		for j in range(len(column_has_number)):
			# if j == 0:
			# 	new_row.append(new_data[j])
			if column_has_number[j]:
				new_row.append(new_data[j])
			else:
	   
				new_row_with_nonnumeric_without_first_item.append(new_data[j])
				
		clean_data.append(new_row)
		nonnumeric_data.append(new_row_with_nonnumeric_without_first_item)
	# pass
	return clean_data, nonnumeric_data, column_dict

def batch_effect_rectification2(input_file, contained_name_with_colname_rowname, data_orient_colname_rowname, parametric_method):
	# 参数判断
	...
	# 	nonnumeric_data.append(new_row_with_nonnumeric_without_first_item)
			# print(new_row)
	clean_data, nonnumeric_data, column_dict = parse_data_2_numeric_nonnumeric(data)
	data = clean_data
	############################# 
	if data_orient_colname_rowname == "行":
	...
	# norm.transform(data)
	# print("norm >>>", result)
	if data_orient_colname_rowname == "列":
	...
	if data_orient_colname_rowname == "行":
		temp = transform_nested_list(temp)
		# temp = transform_nested_list(temp)
		pass

	for row_index in range(len(temp)):
		temp[row_index].extend(nonnumeric_data[row_index])
	

	# test
	swap_columns(temp, 0, 1)
	swap_columns(temp, 2, 1)
	# 排序
	new_header = temp[0]
	for key, column in enumerate(column_dict):
		print(key, column_dict.get(column))
		column_index = column_dict[column]
		transformed_column_index = new_header.index(column, key)
		if column_dict[column] != new_header[key]:
			swap_columns(temp, transformed_column_index, column_index)
...
if __name__ == "__main__":
	...
	batch_effect_rectification2('C:/Users/Cherry/Desktop/归一化_添加新功能/属水平绝对丰度表.tsv',
				# "C:/Users/Cherry/Desktop/geekbang/bioinformatics/learnD3/",
				contained_name_with_colname_rowname ="都有",
				data_orient_colname_rowname="行",
				parametric_method="相对丰度")
