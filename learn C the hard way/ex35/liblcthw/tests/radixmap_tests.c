...
static int test_search(RadixMap *map) {
    ...
    for (i = map->end / 2; i < map->end; i++) {
        ...
        check(found->data.key == d->data.key, "Got the wrong result: %p:%u \
        looking for %u at %u", found, found->data.key, d->data.key, i);
    }
    return 1;
error:
    return 0;
...
static char *test_operations() {
    ...
    mu_assert(map != NULL, "Failed to make ther map.");
    mu_assert(make_random(map), "Didn't make a random fake radix map.");

    // TODO:xiaojiao, [ERROR] (tests/radixmap_tests.c:62: errno: None) Failed to properly sort the Radixmap.
    RadixMap_sort(map);
    ...
    while (map->end > 0) {
        ...
        // test that the end is now thw old value,
