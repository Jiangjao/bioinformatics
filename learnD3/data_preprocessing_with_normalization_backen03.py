...
class MedianFilledLineImputer(Normalization):
        ...
        for line in result_without_header:
            ...
            for i, value in enumerate(values[1:]):
                ...
                if len(id_cols_index) == 0:
                    ...
                    if is_number_flag:
                        counts[gene][sample] = counts[gene].get(sample, 0) + Decimal(value)
                    else:
                        # 添加 同类别分行
    ...
    def get_stastics(self, matrix, parametric_method, colnames, flag):
        ...
        for index in range(0, transposed_matrix_length):
            ...
            row_values = [Decimal(value) for value in row_values if is_number(value)]  # 转换为整数（如果适用）
            if len(row_values) == 0:
                continue

            # 选中指定的列
            if index == flag:
                if parametric_method == "median":
                    test_min_value = get_min_max_value(row_values, statistics.median_low)
        ...
        for index in range(0, transposed_matrix_length):
            ...
            if parametric_method == "avg":
                ...
                if index in id_cols_index:
                    element_average = sep.join(set(row_values))
                elif is_list_full_number:
                    row_values = [Decimal(value) for value in row_values_original if is_number(value)]  # 转换为整数（如果适用）
...
class NormalizationFactory:
    ...
    normalizer_map = {
        ...
        'max':MaxFilledByLineImputer,
        'sum': SumFilledByLineImputer,
        'median': MedianFilledLineImputer,
...
def process_value_by_each_line_with_substring(counts, gene, sample, value, random_string):
    ...
    value_pre = counts[gene].get(sample, "")
    if value != "":
        value = sep.join(set([i.strip() for i in value.split(sep)]))
    # if value not in value_pre.split(sep):
    try:
...
def data_preparation2(data, contained_name_with_colname_rowname):
    """
    洗数据，第二步
    """
...
def normalization_or_standardizatation(input_file, contained_name_with_colname_rowname, category_column_name, parametric_method, na_format, na_value):
    ...
    parametric_method_choices = ['min', 'max', 'sum', 'avg', 'median', 'colMax', 'rowMin', 'rowAvg', 'rowMedian']
    ...
    # 更新数据
    data, colnames, rownames = data_preparation2(data, contained_name_with_colname_rowname)
    ...
    norm = norm.get_normalizer(parametric_method)
    # temp0 = 
    # 4. 计算统计指标
    # 4.1 计算每行的最大值、最小值和总和
    # 4.2 选中指定的列
