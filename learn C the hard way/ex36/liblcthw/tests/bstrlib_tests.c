...
// https://github.com/preslavmihaylov/learn-c-the-hard-way/blob/master/36.SaferStrings/liblcthw/tests/bstrlib_tests.c
...
char *bfromcstr_test() {
    ...
    mu_assert(strcmp("", cstr) == 0,
                "failed to return proper cstring");
...
char *bfromcstralloc_test() {
    ...
    // bstring b = bfromcstralloc(len + 1, str);

    // // 使用bstring
    // printf("bstring: %s\n", b->data); 
    // printf("bstring: %d\n", len); 

    // bdestroy(b); // 释放bstring
    bstr = bfromcstralloc(4, "hell");
    cstr = bstr2cstr(bstr, 5);
    // printf("cstr: <%s>\n", cstr);
...
char *bdata_char_test() {
    ...
    cstr = bdata(bstr);
    mu_assert(strcmp("Hello world", cstr) == 0, "failed data");
