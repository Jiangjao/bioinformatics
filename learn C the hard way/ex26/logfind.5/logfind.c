...
int list_files(glob_t *pglob) {
    ...
    check(pglob != NULL, "Invalid golb_t given.");
    check_mem(line);
    ...
    while (fgets(line, MAX_LINE - 1, file) != NULL) {
        ...
        // dumb work around to as stupid design in glob
        if (glob_flags == GLOB_TILDE) glob_flags |= GLOB_APPEND;
    }

    for (i = 0; i < pglob->gl_pathc; i++) {
        debug("Match file: %s",pglob->gl_pathv[i]);
    }
...
int parse_args(int *use_or, int *argc, char **argv[]) {
    ...
    if (strcmp((*argv)[0], "-o") == 0) {
        *use_or = 1;
        ...
        // bug here...But it will not affect the program's operation
        use_or = 0;
    }
...
int main(int argc, char *argv[]) {
    ...
    check(list_files(&files_found) == 0 , "USAGE: logfind [-o] words");
